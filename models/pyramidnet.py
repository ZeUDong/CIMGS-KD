...
class Bottleneck(nn.Module):
    ...
    def forward(self, x):
        ...
        if residual_channel != shortcut_channel:
            padding = torch.autograd.Variable(torch.cuda.FloatTensor(batch_size, residual_channel - shortcut_channel, featuremap_size[0], featuremap_size[1]).fill_(0)) 
            #padding = torch.FloatTensor(batch_size, residual_channel - shortcut_channel, featuremap_size[0], featuremap_size[1]).fill_(0)
            out += torch.cat((shortcut, padding), 1)
        else:
            out += shortcut 
...
def pyramidnet200(num_classes):
    return PyramidNet(dataset='cifar100', depth=200, alpha=240, num_classes=num_classes, bottleneck=True)

if __name__ == '__main__':
    net = pyramidnet200(num_classes=100)
